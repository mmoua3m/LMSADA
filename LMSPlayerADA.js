  ...
  function transcriptButton(){    
    ...
    const transcriptButton = myPlayer.getChild('ControlBar').addChild('button', {
      ...
      clickHandler: function(event) { //What the button does upon being clicked
        newWindow = window.open(transcriptURL.src.replace("http://", "https://"), null, "height=400,width=600,status=yes,toolbar=no,menubar=no,location=no,toolbars=no,scrollbars=yes,resizable=yes")
      }
    })
    }
