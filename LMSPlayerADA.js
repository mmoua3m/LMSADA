videojs.registerPlugin('LMSPlayerADA', function() {
  ...
  console.log("LMSPlayerADA plugin updated @5/6/2024 11:58 am")
  ...
  setTimeout( function(){ //Delayed code
    ...
    transcriptURL = myPlayer.mediainfo.transcripts[0]
    console.log(transcriptURL.src)
  ...
  // Custom keyboard shortcuts for 'd' and 't' for switching between described video and opening transcript
  addEventListener("keydown", (event) => {});

  onkeydown = (event) => {
    if (event.key == 'd') {
      if ( watchingOriginalVideo == true ) { // If we have the original video loaded and we click the button, load the described video
        myPlayer.getChild('ControlBar').getChild('dvButton').controlText('Original Video') // Switch button text
        myPlayer.catalog.getVideo(describedVideoID, function(error, video) {
          setTimeout( function(){
            myPlayer.catalog.load(video);
            myPlayer.play();
        ...
        myPlayer.getChild('ControlBar').getChild('dvButton').controlText('Described Video') // Switch button text
        myPlayer.catalog.getVideo(originalID, function(error, video) {
          setTimeout( function(){
            myPlayer.catalog.load(video);
            myPlayer.play();
      ...
      if (event.key == 't') {
        newWindow = window.open(transcriptURL.src.replace("http://", "https://"), null, "height=540,width=960,status=yes,toolbar=no,menubar=no,location=no,toolbars=no,scrollbars=yes,resizable=yes")
      }
  };

  // Transcript button code
  function transcriptButton(){    
    // Creates a button that opens a new stripped down window with the transcript text when clicked.
    ...
    // Set cations to autodisplay, type has to == 'subtitles" and not "captions". Check for english and captions/subtitles
    myPlayer.on("loadedmetadata", function () {
      // +++ Display English captions +++
      // Open the English version captions
      for (var i = 0; i < (myPlayer.textTracks().length); i++) {
        if ((myPlayer.textTracks()[i].kind == "subtitles" && myPlayer.textTracks()[i].id == "English (US)") || (myPlayer.textTracks()[i].kind == "subtitles" && myPlayer.textTracks()[i].id == "English (United States)")) {
